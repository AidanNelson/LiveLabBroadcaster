  ...
  useEffect(() => {
    ...
    const handleDisplayNameUpdates = (data) => {
      setDisplayNames((prevDisplayNames) =>
        prevDisplayNames.map((dn) => (dn.id === data.new.id ? data.new : dn)),
      );
    };
    const handleDisplayNameDeletions = (data) => {
      setDisplayNames((prevDisplayNames) =>
        prevDisplayNames.filter((dn) => dn.id !== data.old.id),
      );
    };
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "display_names" },
        handleDisplayNameInserts,
      )
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "display_names" },
        handleDisplayNameUpdates,
      )
      .on(
        "postgres_changes",
        { event: "DELETE", schema: "public", table: "display_names" },
        handleDisplayNameDeletions,
      )
      .subscribe();
