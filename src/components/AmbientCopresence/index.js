...
export const AmbientCopresenceOverlay = ({ isVisible }) => {
  ...
  return (
    ...
    >
      {audience &&
        audience.map((id, index) => {
          if (!id) return null; // Skip if id is null or undefined
          return (
            <div
              key={id}
              className={`${styles.audienceMember} ${
                id === socket.id ? styles.myAudienceMember : ""
              }`}
              style={{ left: getAudienceMemberPosition(index) }}
            >
              <IoEllipseOutline />
            </div>
          );
        ...
        return (
          <div
            ...
            className={`${styles.floatingEmote} ${
              emote.from === socket.id ? styles.myFloatingEmote : ""
            }`}
            style={{
              left: getAudienceMemberPosition(index, emote.randomOffset),
              rotate: `${emote.randomOffset / 5}deg`,
            }}
          >
